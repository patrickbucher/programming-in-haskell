...
Ex. 5) Without using any other library functions or operators, show how the
...
`&&` can be formalised using conditional expressions:
    ...
    conjunction a b = if a then if b then True else False else False

Ex. 6) Do the same for the following alternative definition, and note the
difference in the number of conditional expressions that are required:
    ...
    conjunction :: Bool -> Bool -> Bool
    conjunction a b = if a then b else False

Ex. 7) Show how the meaning of the following curried function definitions can
...
Ex. 8 The _Luhn algorithm_ is used to check bank card numbers for simple errors
